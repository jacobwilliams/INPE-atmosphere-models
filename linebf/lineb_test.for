	...
	WRITE(8, 1020)
	  ...
	  ALTU = IJ
        DO II = 1, 5
          TEXO = T(II)
          CALL PMOWEI (TEXO,ALTU,AN,WMOL,RHOD)   
          RD(II)  = LOG10(RHOD)
	    WM(II)  = WMOL
        ENDDO
	...
	*         RD(3), WM(4), RD(4), WM(5), RD(5)
	...
	WRITE(8, 1050)
      DO IJ = 90, 2000, 10
	  ALTU = IJ
        CALL PSMADE (ALTU*1000., SF, TE, AN, WMOL, RHOD)   
	...
	WRITE(8, 1080) (INT(RL(I)), I = 1, 5)
	  ...
	  DO II = 1, 5
	    SA(2) = RL(II)*3.141592/180.
          CALL PSDAMO (SA, SU, SF, RJUD, DAFR, GSTI, TE, AN, WMOL, RHOD)
	    WM(II) = WMOL
	    RD(II) = LOG10(RHOD)
	  ENDDO   
